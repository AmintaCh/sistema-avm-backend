...
import { BeneficiariosDetalleService } from './beneficiarios-detalle.service';

@Controller('beneficiarios-detalle')
export class BeneficiariosDetalleController {
  constructor(private readonly service: BeneficiariosDetalleService) {}
  ...
  async listar(
    ...
    @Query('es_mayor_edad') mayorEdadSnake?: string, // alias
  ...
  async exportCsv(
    ...
    const filename = `beneficiarios-detalle_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.csv`;
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    // Agregamos BOM para compatibilidad con Excel
    res.send('\uFEFF' + csv);
  }
  ...
  async exportXlsx(
    ...
    const filename = `beneficiarios-detalle_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.xlsx`;
    res.setHeader(
      'Content-Type',
